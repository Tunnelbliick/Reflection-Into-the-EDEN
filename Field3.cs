...
{
    public class Field3 : StoryboardObjectGenerator
    {
        ...
        public override void Generate()
        {
            ...
            // General values
            var starttime = 79630; // the starttime where the playfield is initialized
            var endtime = 84738; // the endtime where the playfield is nolonger beeing rendered
            ...
            var travelDistance = 85f;

            field.Scale(OsbEasing.None, starttime + 1, starttime + 1, new Vector2(0.4f));
            field.moveFieldX(OsbEasing.None, starttime, starttime, -150);


            field.moveFieldX(OsbEasing.InSine, 80721 + 70, 81812 - 10, -travelDistance + 40f);

            var s = 80721;
            var d = 81812 - 80721;
            s += d;

            field.moveFieldX(OsbEasing.None, s, s + d, -travelDistance);
            s += d;
            field.moveFieldX(OsbEasing.None, s, s + d, -travelDistance);
            s += d;
            field.moveFieldX(OsbEasing.None, s, s + d, -travelDistance);
            ...
            field.columns[ColumnType.one].receptor.renderedSprite.Fade(OsbEasing.InSine, 79630, 80721, 0, 1);
            field.columns[ColumnType.two].receptor.renderedSprite.Fade(OsbEasing.InSine, 79630, 80721, 0, 1);
            field.columns[ColumnType.three].receptor.renderedSprite.Fade(OsbEasing.InSine, 79630, 80721, 0, 1);
            field.columns[ColumnType.four].receptor.renderedSprite.Fade(OsbEasing.InSine, 79630, 80721, 0, 1);
